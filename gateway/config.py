...
AUTH_CODE_PATH = DATA_DIR / "auth_code.json"
...
def save_auth_code(payload: dict[str, Any]) -> None:
    ensure_data_dir()
    AUTH_CODE_PATH.write_text(json.dumps(payload, indent=2))


def load_auth_code() -> dict[str, Any]:
    ensure_data_dir()
    if AUTH_CODE_PATH.exists():
        try:
            return json.loads(AUTH_CODE_PATH.read_text())
        except Exception:
            return {}
    return {}
